...
public class SystemJobs {
   ...
   // public List<Operation> getallsystemoperations(){
        //Operation.builder().id(1).OperationName("Hello").Createdat(LocalDateTime.now()).build(),
   ...
   public List<Operation> getAllSystemOperations() {
    ...
    public void loadAllOperations(){
           ...
           for(Operation op:operations){
               ...
               //op.setCreatedBy(SystemConstants.APPLICATION_USER_NAME);
               //op.setUpdatedBy(SystemConstants.APPLICATION_USER_NAME);
               SystemUtilities.setGlobalRecordValue(op,SystemConstants.APPLICATION_USER_NAME,SystemConstants.APPLICATION_USER_NAME,LocalDateTime.now(),LocalDateTime.now());
               //operationRepo to save the object

               Optional<Operation> opr= operationRepository.findByOperationName(op.getOperationName());
               if(opr.isEmpty())
               {
                   operationRepository.save(op);
                   log.info(String.format("Operations with name %s saved in the operations table",op.getOperationName()));
               }
               else{
                   log.info(String.format("Operations with name %s already exits",op.getOperationName()));
               }
           }
    }
}
